...
async def delete_user_me(current_user: CurrentUser) -> Any:
    ...
    await ChatMessageDB.find(
        ChatMessageDB.conversation_id.is_in(  # type: ignore[attr-defined]
            [
                c.id
                for c in await ChatConversation.find(
                    ChatConversation.owner_id == current_user.id
                ).to_list()
            ]
        )
    ).delete()
    await ChatConversation.find(ChatConversation.owner_id == current_user.id).delete()
    await Country.find(Country.owner_id == current_user.id).delete()
...
async def read_user_by_id(user_id: uuid.UUID, current_user: CurrentUser) -> Any:
    """
...
async def delete_user(current_user: CurrentUser, user_id: uuid.UUID) -> Message:
    """
    Delete a user.
    """
    user = await User.get(user_id)
    ...
    await ChatMessageDB.find(
        ChatMessageDB.conversation_id.is_in(  # type: ignore[attr-defined]
            [
                c.id
                for c in await ChatConversation.find(
                    ChatConversation.owner_id == user_id
                ).to_list()
            ]
        )
    ).delete()
    await ChatConversation.find(ChatConversation.owner_id == user_id).delete()
    await Country.find(Country.owner_id == user_id).delete()
    await user.delete()
    return Message(message="User deleted successfully")
